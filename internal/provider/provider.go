	...
	resp.Schema = schema.Schema{
		Attributes: map[string]schema.Attribute{
			...
			"token": schema.StringAttribute{
				Description: "A InfluxDB token string",
				Optional:    true,
				Sensitive:   true,
			},
		},
...
func (p *InfluxDBProvider) Configure(ctx context.Context, req provider.ConfigureRequest, resp *provider.ConfigureResponse) {
	...
	if config.URL.IsUnknown() {
		resp.Diagnostics.AddAttributeError(
			...
			"The provider cannot create the InfluxDB client as there is an unknown configuration value for the InfluxDB URL. "+
				"Either target apply the source of the value first, set the value statically in the configuration, or use the INFLUX_URL environment variable.",
		)
	}
	...
	if config.Token.IsUnknown() {
		resp.Diagnostics.AddAttributeError(
			...
			"The provider cannot create the InfluxDB client as there is an unknown configuration value for the InfluxDB Token. "+
				"Either target apply the source of the value first, set the value statically in the configuration, or use the INFLUX_TOKEN environment variable.",
		)
	}
	...
	if url == "" {
		resp.Diagnostics.AddAttributeError(
			...
			"The provider cannot create the InfluxDB client as there is a missing or empty value for the InfluxDB URL. "+
				"Set the host value in the configuration or use the INFLUX_URL environment variable. "+
	...
	if token == "" {
		resp.Diagnostics.AddAttributeError(
			...
			"The provider cannot create the InfluxDB client as there is a missing or empty value for the InfluxDB Token. "+
				"Set the host value in the configuration or use the INFLUX_TOKEN environment variable. "+
				"If either is already set, ensure the value is not empty.",
		)
	}
	...
	ctx = tflog.SetField(ctx, "INFLUX_URL", url)
	ctx = tflog.SetField(ctx, "INFLUX_TOKEN", token)
	ctx = tflog.MaskFieldValuesWithFieldKeys(ctx, "INFLUX_TOKEN")
